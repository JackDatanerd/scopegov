...
export interface AuditReportData {
  ...
  generatedBy:   string
  generatedAt:   string
  /** IANA zone the workspace uses; times are rendered in it (UTC when omitted). */
  timeZone?:     string
  from:          string
  to:            string
  filters: {
...
// Timestamps are shown in the workspace's own timezone and always carry its
// label (e.g. "EAT" or "UTC"), so a reader can tell which zone they are in.
      ...
      <Page size="A4" style={s.page} wrap>
        <View style={s.header} fixed>
          <View>
            ...
            <Text style={s.meta}>{fmtDate(data.from, data.timeZone)} — {fmtDate(data.to, data.timeZone)}</Text>
          </View>
          <View>
            <Text style={s.metaRight}>Generated by {data.generatedBy}</Text>
            <Text style={s.metaRight}>{fmtDateTime(data.generatedAt, data.timeZone)}</Text>
